...
HISTSIZE=4096
SAVEHIST=4096
HSLINES=$(cat ~/.bash_history_summary|sort -k1|uniq -f1|wc -l)
export ERL_AFLAGS="-kernel shell_history enabled"
cat ~/.zhistory|sort -k1|uniq -f1 >> ~/.bash_history_summary
if [ $HSLINES!=0 ];then
   # using subshell
   echo "$(cat ~/.bash_history_summary|sort -k1|uniq -f1)" > ~/.bash_history_summary
